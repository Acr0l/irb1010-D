...
def extend_vector(x1, y1, x2, y2, D):
    ...
    uVx, uVy = Vx / mag_V, Vy / mag_V
    
    # Extend the vector by distance D
    Cx, Cy = x2 + D * uVx, y2 + D * uVy
...
while(True): 
   ...
   #Se repite el proceso, ahora para encontar los arcos
    ...
    p_alineacion = extend_vector(x1a, (y1a+y2a)//2, yellow_c[0], yellow_c[1], 10)

    cv2.circle(img, (int(p_alineacion[0]), int(p_alineacion[1])), 5, (0, 0, 255), -1)
    ...
    if en_posicion:
        ...
        current_angle = calculate_angle_between_points(blue_c, red_c)
        target_angle = calculate_angle_between_points(blue_c, p_alineacion)
        target = p_alineacion
